...
def train(args):
    ...
    checkpoints = tf.keras.callbacks.ModelCheckpoint(filepath=args.checkpoint_dir, save_weights_only=True, 
                                            save_freq=args.save_freq, monitor='val_loss', mode='min', verbose=1,save_best_only=True)
    model.fit(ds_train, validation_data=ds_valid, epochs=args.num_epochs, callbacks=[early_stopping, checkpoints])

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--data_path', default="data/complete/")
    parser.add_argument('--checkpoint_dir', default="/content/drive/MyDrive/pcn_tf_2/checkpoint")
